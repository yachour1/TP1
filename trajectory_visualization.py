...
class Evaluate():
    ...
    def main(self, name, val):
        ...
        l_path3 = '/usagers3/yaach/Documents/HLTP/checkpoints/model'
        generator = student_model.Generator_stu(args=args)
        gdEncoder = student_model.GDEncoder_stu(args=args)
        ...
        generator.eval()
        gdEncoder.eval()
        net = teacher_model.highwayNet(args=args) #.v860.
        net.load_state_dict(t.load(l_path2 + '/epoch' + name2 + '.pth', map_location=torch.device('cuda:0')))
        net = net.to(device)
        net.eval()
        stdan_gen = student_model.Generator_stu(args=args) #student_model_original
        stdan_gde = student_model.GDEncoder_stu(args=args) #student_model_original
        stdan_gen.load_state_dict(t.load(l_path3 + '/epoch' + name3 + '_g.pth', map_location='cuda:0'))
        stdan_gde.load_state_dict(t.load(l_path3 + '/epoch' + name3 + '_gd.pth', map_location='cuda:0'))
        stdan_gen = stdan_gen.to(device)
        stdan_gde = stdan_gde.to(device)
        stdan_gde.eval()
        stdan_gen.eval()
        valSet = lo.ngsimDataset('./NGSIM/data/dataset_t_v_t/TestSet.mat')
        ...
        with(t.no_grad()):  
            for idx, data in enumerate(valDataloader):
                ...
                data_loading_time = time.time() - previous_time
                print(f"Time to load batch {idx}: {data_loading_time:.4f} seconds")
                ...
                values = gdEncoder(hist_stu, nbrs_stu, mask, va_stu, nbrsva_stu, lane_stu, nbrslane_stu, cls_stu,
                                      nbrscls_stu)  #values, s 
            ...
            if args['val_use_mse']:  
                ...
                print(f"Time to process batch {idx}: {batch_processing_time:.4f} seconds")
    ...
    def draw(self, hist, fut, nbrs, mask, fut_pred1, fut_pred2, fut_pred3, train_flag, lon_man, lat_man, op_mask,
        ...
        for i in range(hist.size(1)):
            ...
            if lat_man_i != 0:
                ...
                directory = './pic/' + str(lon_man_i + 1) + '_' + str(lat_man_i + 1)
                ...
                # Save the figure
                plt.savefig(directory + '/' + str(self.op) + '.png')
                self.op += 1
